...
class SCZCTableViewController: UITableViewController {
    ...
    let ref = Database.database().reference(withPath: "sczc-items")
    ...
    override func viewDidLoad() {
        ...
        ref.queryOrdered(byChild: "completed").observe(.value, with: { snapshot in
            var newItems: [SCZCItem] = []
            for child in snapshot.children {
                if let snapshot = child as? DataSnapshot,
                    let sczcItem = SCZCItem(snapshot: snapshot) {
